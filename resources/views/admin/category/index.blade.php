							...
							<tr>
								...
								<td>
									...
									<form id="deleteCategory_{{$row->category_row_id}}" action="{{ url('/')}}/admin/category/{{$row->category_row_id}}" style="display: inline;" method="POST">
										{{ method_field('DELETE') }}
										@csrf
										<input class="btn btn-danger deleteLink" category_name="{{ $row->category_name }}" category_row_id="{{$row->category_row_id}}" data-toggle="modal" data-target="#modal-danger" deleteID="{{$row->category_row_id}}" value="Delete" style="width: 100px;">
									</form>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('.deleteLink').click(function() {
			...
			$('#modal-danger .catname').append(category_name);
			$('#modal-danger .submitDeleteModal').attr('category_row_id', category_row_id)
		});
